...
def time_cal():
  #  x = time1_entry.get()
  #  y = time2_entry.get()
    timex = datetime.strptime(time1_entry.get() ,"%H:%M:%S")

    timey = datetime.strptime(time2_entry.get() ,"%H:%M:%S")
...
header_label = tk.Label(window, text='Time calculator',font=fontExample)
header_label.pack()
